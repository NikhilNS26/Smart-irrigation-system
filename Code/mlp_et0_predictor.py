...
def predict_et0(raw_features):
    ...
    # Note: Weights are often stored as (inputs, neurons). We need (neurons, inputs) for mat_vec_mult.
    # Scikit-learn coefs_ are [n_features, n_neurons]. Let's transpose conceptually.
    # Or adjust matrix_vector_multiply. Let's adjust the function.

    # Assuming WEIGHTS_0 is [n_features, n_neurons_layer1]
    # We need to compute dot(input, weights_for_neuron) + bias_for_neuron for each neuron

    # Hidden Layer 1 Calculation
    layer1_neurons = len(BIASES_0)
    layer1_output = [0.0] * layer1_neurons
    for i in range(layer1_neurons): # For each neuron in layer 1
        neuron_input_weights = [WEIGHTS_0[j][i] for j in range(len(scaled_input))] # Get weights for this neuron
        activation = dot_product(scaled_input, neuron_input_weights) + BIASES_0[i]
        layer1_output[i] = relu(activation) # Apply ReLU

    # Hidden Layer 2 Calculation (if exists - adjust based on extracted params)
    # Assuming WEIGHTS_1 is [n_neurons_layer1, n_neurons_layer2]
    # Input to this layer is layer1_output
    layer2_neurons = len(BIASES_1)
    ...
    for i in range(layer2_neurons): # For each neuron in layer 2
        ...
        # Check if this is the *output* layer. MLPRegressor output is identity.
        # If there were more hidden layers, apply relu here.
        # Since BIASES_1 is the last bias array, this is the output layer.
        layer2_output[i] = identity(activation) # Apply Identity for output layer

    # Final output is the result from the last layer
    # Assuming a single output neuron as is typical for regression
    if len(layer2_output) != 1:
         print("Warning: Expected a single output neuron for regression.")
         # Handle multiple outputs if necessary, e.g., return the list or average
         return layer2_output[0] # Return the first output if unsure

    return layer2_output[0]
